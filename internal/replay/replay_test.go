...
// Hierarchical matching: exact wins when bodies are deterministic; shape
// rescues per-transaction variance (amounts differ); sequence is the last
// resort for unseen bodies on a known template.
func TestHierarchicalMatching(t *testing.T) {
	...
	// Tier 1: identical body (fixture determinism).
	...
	// Tier 2: same shape, different amount — the real-traffic case.
...
// MatchValue (the sandbox engine's entry point) matches parsed bodies and
// names the tier; a miss returns nil so the engine can fall through to 404.
func TestMatchValueServesParsedBodies(t *testing.T) {
	...
	// Loaded numbers are json.Number now (lossless precision) — the served
	// bytes are what matter, and json.Number marshals verbatim.
...
// CI gate: recordings that diverge from the frozen baseline yield findings;
// clean recordings yield none.
func TestGateFindsOfflineDrift(t *testing.T) {
	dir := t.TempDir()

	// Build + freeze a baseline by observing clean traffic.
	...
	// Recordings: two clean, one drifted (renamed enum + extra field).
	writeRecordings(t, dir, "pay", []proxy.Record{
	...
	// Clean-only recordings → zero findings.
	writeRecordings(t, dir, "pay", []proxy.Record{
...
// Degraded serves and misses carry match-phase diagnostics.
func TestMatchValueDiag(t *testing.T) {
	...
	// Shape serve: names the VALUE that kept exact from matching — including
	// a numeric one (baseline.Flatten only carries string values; the gap
	// flatten must not inherit that blindness).
	...
	// Sequence serve: names the field-set gap.
	...
	// Full miss: the closest RECORDED endpoint, read-only re-lookup agrees.
...
// The same exact request recorded N times with different responses
// replays as an ordered sequence, then HOLDS the last — never degrading to
// the shape tier for a request the exact tier owns.
func TestExactTierSequenceVariants(t *testing.T) {
	...
	wantStatuses := []int{503, 503, 200, 200, 200} // order, then hold the LAST
	...
	// A DIFFERENT request body owns its own sequence, untouched.
	...
	if rec, _ := s.MatchValueDiag("POST", "/charge", other); rec == nil || rec.RespBody.(map[string]any)["attempt"] != float64(1) {
		// shape tier serves the first unserved... but all are served, so
		// sequence-tier wrap applies — either way it must not panic and must
		// serve SOMETHING for the same endpoint.
		if rec == nil {
...
// A single recording has no sequence to report.
func TestExactTierSingleRecordingNoSequence(t *testing.T) {
	...
	// Re-request: still exact (held), not degraded.
...
// Exact-tier keys are format-blind on numbers: a recorded "500.00" and a
// live 500 share an identity; >2^53 literals keep exact digits.
func TestExactKeyNumberCanonicalization(t *testing.T) {
	...
	// Live body parses "500" as float64 — must still hit the exact tier.
	...
	// Distinct huge literals stay distinct (no float64 collapse).
